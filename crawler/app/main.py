...
def get_city_info(city_id):
    ...
    logging.info("City Info: {}".format(city_info))
...
def update_db(db_session, biz_circle, communities):
    ...
    for community_info in communities['list']:
        try:
            # if (community_info['district_name'] == '杨浦'):
            ...
            db_session.query(Community).filter(
                Community.id == community_info['community_id']
            ).delete()
                # logging.info('种类：{}; 小区 id: {}'.format(type(community_info['community_id']), community_info['community_id']))
                # if community_info['community_id'] != '5011000014141':
